...
while sample!="END":
    puntos=[]
    figura=sys.stdin.readline().strip()
    print(figura)
    while figura[0]=="P":
        figura=figura.split()[1:]
        for i in range(int(figura[0])):
            x=float(figura[2*i+1])
            y=float(figura[(2*i)+2])
            puntos.append(point(x,y))
        figura=sys.stdin.readline().strip()
    print(puntos)
    hull_encontrado=hull(puntos)
    print("hull encontrado: ",hull_encontrado)
