...
def encontrar_divisores(numero):
    ...
    return divisors
i=1
c,r,o=map(int,sys.stdin.readline().strip().split())
while r!=0 and c!=0 and o!=0:
    puntos_policias=[]
    for i in range(c):
        x,y=map(float,sys.stdin.readline().strip().split())
        puntos_policias.append(point(x,y))
    cuadrante_policias=hull(puntos_policias)
    puntos_ladrones=[]
    for i in range(r):
        x,y=map(float,sys.stdin.readline().strip().split())
        puntos_ladrones.append(point(x,y))
    cuadrante_ladrones=hull(puntos_ladrones)
    ciudadanos=[]
    for i in range(o):
        x,y=map(float,sys.stdin.readline().strip().split())
        ciudadanos.append(point(x,y))
    print(f"Data set {i}:")
    for citzen in ciudadanos:
        if cuadrante_policias.is_inside(citzen):
            print(f"Citizen at ({int(citzen.x)},{int(citzen.y)}) is safe.")
        elif cuadrante_ladrones.is_inside(citzen):
            print(f"Citizen at ({int(citzen.x)},{int(citzen.y)}) is robbed.")
        else:
            print(f"Citizen at ({int(citzen.x)},{int(citzen.y)}) is neither.")
    i+=1
