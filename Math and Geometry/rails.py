...
class segment:
    ...
    def distancia_a_un_punto(self,p):
        ...
        pendiete=liseg.slope()
        if (pendiete==0):
            slope=math.inf
        else:
            slope=-1/pendiete
        linea_de_intersecccion=line.from_point_slope(p,slope)
...
def encontrar_divisores(numero):
    ...
    divisors.sort()
    return divisors
Xm=sys.stdin.readline().strip()
puntos=[]
while Xm !="":
    Xm=float(Xm)
    Ym=float(sys.stdin.readline())
    M=point(Xm,Ym)
    ...
    for i in range((N+1)):
        ...
        if(i==0):
            ...
            punto_actual=point(X,Y)
            segmentos.append(segment(punto_anterior,punto_actual))
            punto_anterior=punto_actual
    min_distacia=None
    punto_minimo=None
    for segmento in segmentos:
        if(min_distacia==None or segmento.distancia_a_un_punto(M)<min_distacia):
            min_distacia=segmento.distancia_a_un_punto(M)
            punto_minimo=segmento.punto_de_minima_distancia(M)

    puntos.append(punto_minimo)
