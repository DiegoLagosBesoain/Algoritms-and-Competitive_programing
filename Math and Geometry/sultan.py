...
yf=float(l[4])
fuente=point(xf,yf)
cuadrilatero=polygon([point(0,0),point(0,H),point(W,H),point(W,0)])
jardin_con_fuente=cuadrilatero
for i in range(N):
    l = sys.stdin.readline().split()
    segemento=segment(point(float(l[0]),float(l[1])),  point(float(l[2]),float(l[3]))          )
    j1,j2 =jardin_con_fuente.polygon_split(segemento)
    if j1.is_inside(fuente):
        jardin_con_fuente=j1
    else:
        jardin_con_fuente=j2
    
print(abs(jardin_con_fuente.area))
