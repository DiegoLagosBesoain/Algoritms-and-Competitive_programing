...
def laser_tapado(punto_actual, puntos , totem):
    linea_del_laser=line.from_points(punto_actual[0],totem)
    xmax=max(punto_actual[0].x,totem.x)
    xmin=min(punto_actual[0].x,totem.x)
    ymax=max(punto_actual[0].y,totem.y)
    ymin=min(punto_actual[0].y,totem.y)
    for punto in puntos:
        if punto[0]!=punto_actual[0]:
            if punto[0].x>=xmin and punto[0].x<=xmax and punto[0].y>=ymin and punto[0].y<=ymax:
                if(linea_del_laser.a*punto[0].x +linea_del_laser.b*punto[0].y+ linea_del_laser.c < EPS):
                    if punto[1]>=punto_actual[1]:
                        return True
                    
    return False
def revisar_datset(puntos):
    tapados=[]
    for punto in puntos:
        if (laser_tapado(punto,puntos,point(0,0))):
            print("entre")
            tapados.append(punto)
    return tapados
...
while N!=0:
    ...
    for i in range(N):
        ...
        puntos.append([point(int(l[0]),int(l[1])),int(l[2])])
    tapados=revisar_datset(puntos)
    print(tapados)
    N = int(sys.stdin.readline())
