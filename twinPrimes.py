...
prime_iter=primes()
primo_anterior=next(prime_iter)
while N != "":
    N=int(N)
    if largo_mem<N:
        while largo_mem!=N:
            
            primo_actual=next(prime_iter)
        
            if primo_actual==primo_anterior +2:
                mem.append([primo_anterior,primo_actual])
                largo_mem+=1
            
            primo_anterior=primo_actual

    print(f"({mem[N-1][0]}, {mem[N-1][1]})")
    N= sys.stdin.readline().strip()
