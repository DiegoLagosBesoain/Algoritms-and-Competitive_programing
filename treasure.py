...
respuestas=[]
while cordenadas!="END":
    x=0
    y=0
    
    cordenadas=cordenadas.replace(".",",").split(",")[:len(cordenadas)-2]
    
    for instruccion in cordenadas:
        if(instruccion==""):
            break
        
        cantidad=int(instruccion[:primer_letra(instruccion)])
        direccion=instruccion[primer_letra(instruccion):]
        if direccion=="N":
            y+=cantidad
        if direccion=="S":
            y-=cantidad
        ...
        if direccion=="E":
            x+=cantidad
        if direccion=="NE":
            y+=cantidad/math.sqrt(2)
            x+=cantidad/math.sqrt(2)
        if direccion=="NW":
            y+=cantidad/math.sqrt(2)
            x-=cantidad/math.sqrt(2)
        if direccion=="SE":
        ...
        if direccion=="SW":
            y-=cantidad/math.sqrt(2)
            x-=cantidad/math.sqrt(2)
    respuestas.append(point(x,y))
    cordenadas=sys.stdin.readline().strip()
for u,i in enumerate(respuestas):
    print(f"Map #{u+1}")
    print(f"The treasure is located at ({ i.x:.3f},{i.y:.3f}).")
    print(f"The distance to the treasure is {i.distance_bewtn(point(0,0)):.3f}.\n")
